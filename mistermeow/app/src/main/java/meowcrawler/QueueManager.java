...
public class QueueManager {
  ...
  private BiMap<String, Queue<Url>>
      domainIndexMp; // Bidirectional-map between url and domain queue in the
                     // list

  private Integer domainQListSize; // Denotes the total number of domain queues
                                   // inside the list
  ...
  public boolean moveToDomainQ() {
    ...
    if (q == null) {
      ...
      if (q == null) {
        u.decrementPriority(); // decreasing priority so that it doesn't get
                               // again into the most prior one
  ...
  public void printPriorityQ() { print(priorityQ); }
...
// implements fn compare to be able to sort Linkedlists inside the priority
// queue
