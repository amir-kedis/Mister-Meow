...
public class QueryRanker extends Ranker {
  ...
  protected List<ObjectId> getMatchingDocs(List<String> searchTokens,
                                           String query) {
    ...
    ProcessedDocs = db.getDocuments2(distinctDocs);

    for (ObjectId docId : distinctDocs) {
      QueryDocRel.add((double)docRepetition.get(docId) /
                      (double)searchTokens.size());
    }
